...
<section class="header">
    ...
    <h1 class="title"><i class="fas fa-burn"></i> GAS BILL </h1>
    ...
    <div id="visible" style="visibility: hidden;">
        ...
        <div></div>
            <p>
                ...
                if(isset($_POST['ss'])){
                $search=$_POST['search'];
                $tsql="SELECT code,phone_number,payment_value FROM gas_bills  WHERE phone_number like '%$search%';";
...
<!-- $tsql="SELECT gas_bills.payment_value AS g_bill,water_bills.payment_value AS w_bill FROM gas_bills,water_bills  WHERE gas_bills.phone_number & water_bills.phone_number like '%$search%';";
                $result=sqlsrv_query($conn,$tsql);
                while($row=sqlsrv_fetch_array($result)){
                echo "<div class='pack' ><h3  >gas_bill: (".$row['g_bill']."L.E)</h3><h3  >water bill: (".$row['w_bill']."L.E)</h3></div>";} -->



<!-- $tsql="SELECT gas_bills.payment_value AS g_bill, water_bills.payment_value AS w_bill, total = gas_bills.payment_value+water_bills.payment_value FROM gas_bills,water_bills  WHERE gas_bills.phone_number & water_bills.phone_number like '%$search%';"; -->
